...
## ⚠️ GitHub Actions 上抓不到（2026-09-06 實測兩次）

**Cloudflare 擋的不只是指紋，還有 IP 網段。** 本機（住宅 IP）四個設定檔都穩定 200，
但同一份程式在 Actions runner（Azure datacenter 網段）上，四個設定檔 ×2 次重試
**全部 403**，連跑兩次結果一樣。

所以這張卡在排程裡的行為是：抓取失敗 → `build.py` 沿用上一版的值並亮黃燈
（note 會寫「4 個設定檔全被擋」＋「本次未取得，沿用 YYYY-MM-DD 的前值」）。
**值要前進得靠本機跑一次 `scripts/build.py` 再推**（MI 每月發布一次，
下次發布日寫在新聞頁的「Next release」）。同指標的季度版走 RBA G3，不受影響，
排程會自己更新。

不要為了讓這張卡變綠而去接第三方鏡像或代理站——寧可黃燈寫清楚原因。
